...
const jsDocPlugin = {
    ...
    run({ sourceFile, text, options }) {
        const result = typescript_1.default.transform(sourceFile, [jsDocTransformerFactory(options)]);
...
const jsDocTransformerFactory = ({ annotateReturns, anyAlias, typeMap: optionsTypeMap, }) => (context) => {
    ...
    function visit(origNode) {
        ...
        if (typescript_1.default.isFunctionLike(node)) {
            return visitFunctionLike(node, typescript_1.default.isClassDeclaration(origNode.parent));
        }
        return node;
    }
    function visitFunctionLike(node, insideClass) {
        ...
        switch (node.kind) {
            case typescript_1.default.SyntaxKind.FunctionDeclaration:
                return factory.updateFunctionDeclaration(node, node.decorators, newModifiers, node.asteriskToken, node.name, node.typeParameters, newParameters, newType, node.body);
            case typescript_1.default.SyntaxKind.MethodDeclaration:
                return factory.updateMethodDeclaration(node, node.decorators, newModifiers, node.asteriskToken, node.name, node.questionToken, node.typeParameters, newParameters, newType, node.body);
            case typescript_1.default.SyntaxKind.Constructor:
                return factory.updateConstructorDeclaration(node, node.decorators, newModifiers, newParameters, node.body);
            case typescript_1.default.SyntaxKind.GetAccessor:
                return factory.updateGetAccessorDeclaration(node, node.decorators, newModifiers, node.name, newParameters, newType, node.body);
            case typescript_1.default.SyntaxKind.SetAccessor:
                return factory.updateSetAccessorDeclaration(node, node.decorators, newModifiers, node.name, newParameters, node.body);
            case typescript_1.default.SyntaxKind.FunctionExpression:
                return factory.updateFunctionExpression(node, newModifiers, node.asteriskToken, node.name, node.typeParameters, newParameters, newType, node.body);
            case typescript_1.default.SyntaxKind.ArrowFunction:
                return factory.updateArrowFunction(node, newModifiers, node.typeParameters, newParameters, newType, node.equalsGreaterThanToken, node.body);
            default:
